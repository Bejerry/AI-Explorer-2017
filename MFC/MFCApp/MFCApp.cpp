...
BEGIN_MESSAGE_MAP(CMFCAppApp, CWinApp)
	...
	ON_COMMAND(ID_MY_TEST, &CMFCAppApp::OnPatternDis)
	ON_COMMAND(ID_APP_TEST, &CMFCAppApp::OnImageTest)
	// 基于文件的标准文档命令
...
BOOL CMFCAppApp::InitInstance()
{
	...
	m_pMainWnd->SetWindowText(L"计算智能-验证测试池");
	return TRUE;
}
...
// CMFCAppApp 消息处理程序
void CMFCAppApp::OnPatternDis()
{
	ImageReader::GetInstance()->testPics(NULL);
	OpenCVFunc::GetInstance()->FuncTest();
}

void CMFCAppApp::OnImageTest()
{
	ImageReco::GetInstance()->DoModal();
}
